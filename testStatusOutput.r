...
#
################################################################################
testStatus <- function (stimResponse,currentNumPres,currentThresholds,finishedThresholds,finished_counter,gp,fp_counter,fn_counter,stateInfo,plotStimResponse=TRUE) {
  ...
  text(1,51,paste("Presentations: ",sum(unlist(currentNumPres),na.rm=TRUE),sep=""),pos=4,cex=2)
  timeStamp <- Sys.time()
  text(1,44,paste("Test Time:",format(.POSIXct(difftime(timeStamp,commence,units="secs") - timePaused),"%M:%S")),pos=4,cex=2)
  text(1,37,paste("% Complete: ",finished_counter,"/",length(which(gp > 0))," (",round(finished_counter/length(which(gp > 0))*100),"%)",sep=""),pos=4,cex=2)
...
testStatusFinal <- function (summary) {
  ...
  text(1,100,paste("ID:",details$name,sep=" "),pos=4,cex=2,font=2)
  text(1,93, ifelse (details$eye == "right","Eye: Right","Eye: Left"),pos=4,cex=2)
  text(1,86,paste("Stimulus Size: ",details$stimSizeRoman,sep=""),pos=4,cex=2)
  text(1,79,paste("Total Test Time:",format(.POSIXct(difftime(terminate,commence,units="secs") - timePaused),"%M:%S")),pos=4,cex=2)
  
  text(1,69,paste(details$grid,"Test",sep=" "),pos=4,cex=2,font=2)
  if (length(summary$fp_counter) > 0){
    text(1,62,paste("FP Rate:",sum(summary$fp_counter,na.rm=TRUE),"/",length(summary$fp_counter),
                    "(",round(sum(summary$fp_counter,na.rm=TRUE)/length(summary$fp_counter)*100),"%)",sep=" "),pos=4,cex=2)
  } else {
    text(1,62,"FP Rate: 0 / 0 (0%)",pos=4,cex=2)
  }
  
  if (length(summary$fn_counter) > 0){
    text(1,55,paste("FN Rate:",sum(summary$fn_counter,na.rm=TRUE),"/",length(summary$fn_counter),
                    "(",round(sum(summary$fn_counter,na.rm=TRUE)/length(summary$fn_counter)*100),"%)",sep=" "),pos=4,cex=2)
  } else {
    text(1,55,"FN Rate: 0 / 0 (0%)",pos=4,cex=2)
  }
  
  text(1,48,paste("Presentations: ",sum(unlist(summary$np),na.rm=TRUE),sep=""),pos=4,cex=2)
  text(1,41,paste("% Complete: ",length(which(!is.na(summary$th))),"/",length(which(!is.na(summary$th)))," (100%)",sep=""),pos=4,cex=2)
  
  if (!is.null(summary$thOutliers)) {
    text(1,28,"Outliers",pos=4,cex=2,font=2)
    text(1,21,paste("Presentations: ",sum(unlist(summary$npOutliers),na.rm=TRUE),sep=""),pos=4,cex=2)
    text(1,14,paste("% Complete: ",length(which(!is.na(summary$thOutliers))),"/",length(which(!is.na(summary$thOutliers)))," (100%)",sep=""),pos=4,cex=2)
  }
