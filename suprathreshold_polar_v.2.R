...
# 
#########################################################################
suprathreshold_PV2 <- function(#eye="right", 
                           # primaryStartValue=30, 
                           # gridType="24-2",
                           practice = FALSE,
  ...
  if (practice)
    ...
    # if (is.matrix(tt)) {
        ...
        #   stop("suprathreshold_PV2(): tt is not a 109*181 matrix")
        # }
      # if (ncol(tt) != 128) {
      #   stop("suprathreshold_PV2(): tt is not a matrix with 128 columns")
      # }
    ...
    #     prior <- prior/sum(prior)
        ...
        # makeStimHelper <- function(x, y, stimSize, responseWindow) {  # returns a function of (db,n)
        #     ff <- function(db, n) {
        #       s <- list(x=x, y=y, level=dbTocd(db,4000/pi),
        #                 size=as.numeric(stimSize),
        #                 color="white",
        #                 duration=200, responseWindow=responseWindow)
        #       
...
# writeFile <- function (filename = paste("../",details$dx,"/",details$gridType," ",details$stimSizeRoman,"/",details$name,"_",details$dx,"_",details$gridType,"_",details$stimSizeRoman,"_",details$eye,"Eye_",details$date,"_",details$startTime,".csv",sep="")) {
  ...
  # cat("\n#X coordinates: ", res$ti$x, "\n", file=filename, append=TRUE)
  # cat("#Y coordinates: ", res$ti$x, "\n", file=filename, append=TRUE)
...
# }
res <- suprathreshold_PV2(practice = FALSE, eyeSuiteSettingsLocation="C:/ProgramData/Haag-Streit/EyeSuite/",gazeFeed=0,bigWheel=TRUE,resp_buzzer = 3)
